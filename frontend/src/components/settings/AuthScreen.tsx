...
const FEATURES = [
  { title: 'Una sola oración', desc: 'Describí la campaña en lenguaje natural — Adkio infiere objetivo, audiencia, copy y presupuesto.' },
  { title: 'Multi-canal', desc: 'Meta, TikTok y Google Ads desde la misma cuenta. Adkio elige la mejor según tu objetivo.' },
  { title: 'Human-in-the-loop', desc: 'Todas las campañas se crean en PAUSED. Vos revisás y aprobás antes de que gaste un peso.' },
  { title: 'Memoria de marca', desc: 'Una vez configurada, Adkio aprende tu tono, tu audiencia y qué copy convierte para vos.' },
];

export default function AuthScreen({ onSuccess, signup, login }: Props) {
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 pt-8 pb-16 grid md:grid-cols-2 gap-12 items-center">
        ...
        <div className="hidden md:block">
          ...
          <p className="mt-5 text-white/55 text-[15px] leading-relaxed max-w-md">
            Creá una cuenta Adkio para conectar Meta, TikTok y Google Ads, y dejar que un agente experto
            configure tus campañas en segundos.
          </p>
        ...
        {/* Right: Form */}
        <div>
          <AuthForm mode={mode} setMode={setMode} onSuccess={onSuccess} signup={signup} login={login} />
        </div>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-white/45 mb-6">
        {mode === 'signup'
          ? 'No te pedimos tarjeta. Free trial de 14 días.'
          : 'Entrá a tu workspace y a tus conexiones.'}
      </p>
